...
function FormD({ showNotification }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3001/getAllMaintain')
      .then(response => {
  ...
  const handleFormSubmit = (index) => {
    ...
    axios.post('http://localhost:3001/updateMaintain', {
      id: updatedForm._id,
  ...
  const handleDelete = (id) => {
    ...
    axios.post('http://localhost:3001/deleteMaintain', {
      id: id,
    })
    .then(response => {
              ...
              <tr key={index}>
                <td>
                  {editedIndex === index ? (
                    ...
                    <span onClick={() => handleCellClick(index)}>{form.vehiclenumber}</span>
                  )}
                </td>
                ...
                <td>
                  {editedIndex === index ? (
                    <input
                      type="date"
                      value={form.maintainsDate}
                      onChange={(e) => handleCellChange(index, 'maintainsDate', e.target.value)} />
                  ) : (
                    <span onClick={() => handleCellClick(index)}>{form.maintainsDate}</span>
                  )}
                </td>
                <td>
                  {editedIndex === index ? (
                    ...
                    <span onClick={() => handleCellClick(index)}>{form.price}</span>
                  )}
                </td>
                <td>
                  {editedIndex === index ? (
