...
function FormD({ showNotification }) {
  ...
  const handleFormSubmit = (index) => {
    ...
    axios.post('http://localhost:3001/updateFormData1', {
      id: updatedForm._id,
...
<p className="applicant-name"> Applicant Name: {editedIndex === index ? (
   ...
   <div className="Sdetails">
              ...
              <p className="expanded-detail">Date Required :
                {editedIndex === index ? (
                  <input type="text" value={form.dateofRequired} onChange={(e) => handleCellChange(index, 'dateofRequired', e.target.value)} />
                ) : (
                  <span onClick={() => setEditedIndex(index)}>{form.dateofRequired}</span>
                )}
              </p>
              
              <p className="expanded-detail">Time Required :
                {editedIndex === index ? (
                  <inpu type="text" value={form.timeofRequired} onChange={(e) => handleCellChange(index, 'timeofRequired', e.target.value)} />
                ) : (
      ...
      <div className="Sdetail-row2">
              ...
              <p className="expanded-detail">Time to be Spent :
                {editedIndex === index ? (
                  <input type="text" value={form.timetobeSpent} onChange={(e) => handleCellChange(index, 'timetobeSpent', e.target.value)} />
                ) : (
              ...
              <p className="expanded-detail">Date Arrival :
                {editedIndex === index ? (
                  <input type="text" value={form.dateofArrival} onChange={(e) => handleCellChange(index, 'dateofArrival', e.target.value)} />
                ) : (
              ...
              <p className="expanded-detail">Time Arrival :
                {editedIndex === index ? (
                  <input type="text" value={form.timeofArrival} onChange={(e) => handleCellChange(index, 'timeofArrival', e.target.value)} />
                ) : (
